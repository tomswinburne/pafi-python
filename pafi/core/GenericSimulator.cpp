...
void GenericSimulator::expansion(double T, double *newscale) {
  /*
    Apply thermal expansion to box. If isotropic coefficients are nonzero 
    they are used, otherwise we use 
  */
  double l_coeff=0.0,q_coeff=0.0;
  if(std::fabs(std::stod(parser->configuration["LinearThermalExpansion"]))>0.) {
    l_coeff = std::stod(parser->configuration["LinearThermalExpansion"]);
    q_coeff = std::stod(parser->configuration["QuadraticThermalExpansion"]);
    for(int i=0;i<3;i++) newscale[i] = 1.0 + l_coeff*T + q_coeff*T*T;
  } else {
